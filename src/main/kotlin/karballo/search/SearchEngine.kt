...
open class SearchEngine(var config: Config) : Runnable {
    ...
    internal var startStopSearchLock = Any()
    ...
    init {
        ...
        for (i in 1..63) {
            for (j in 1..63) {
                logMatrix[i][j] = (Math.log(i.toDouble()) * Math.log(j.toDouble()) / (Math.log(63.0) * Math.log(63.0))).toFloat()
            }
        }
    ...
    fun clearHistory() {
        for (i in 0..5) {
            Arrays.fill(history[i], 0.toShort())
        }
    }
    ...
    fun search(nodeType: Int, depthRemaining: Int, alphaIn: Int, betaIn: Int, allowPrePruning: Boolean, excludedMove: Int): Int {
        ...
        while (true) {
            ...
            if (nodeType != NODE_ROOT
                ...
                if (depthRemaining >= LMR_DEPTHS_NOT_REDUCED) {
                    reduction = (0.5f + 4.4f * (if (nodeType == NODE_NULL) 1f else 0.85f) * (1f - 0.8f * nodeEvalDiff / (NODE_EVAL_DIFF_MAX - NODE_EVAL_DIFF_MIN)) // [-0.5..0.5]
                            * (1f - 1.1f * node.moveIterator.lastMoveScore / (HISTORY_MAX - HISTORY_MIN)) * logMatrix[Math.min(depthRemaining / PLY, 63)][Math.min(moveCount, 63)]).toInt()
            ...
            if (score > bestScore && (config.rand == 0 //... insert errors to lower the ELO
                    ...
                    || random.nextInt(1000) > config.rand)) {
                bestMove = node.move
                bestScore = score
    ...
    private fun prepareRun() {
        ...
        if (config.useBook && config.book != null && board.isUsingBook
                && (config.bookKnowledge == 100 || random.nextFloat() * 100 < config.bookKnowledge)) {
