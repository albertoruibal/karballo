...
class MateTest {
    ...
    internal var searchEngine: SearchEngine = SearchEngine(Config())
    ...
    private fun testPosition(fen: String, move: String) {
        searchEngine.board.fen = fen
        //println("Looking for " + move)
        //System.out.println(moveGenerator.generateMoves(board));
        searchEngine.go(SearchParameters[5000])
        val bestOperation = Move.toString(searchEngine.bestMove)
        //println()
